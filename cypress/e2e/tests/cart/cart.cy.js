...
describe('Cart', () => {
    ...
    it('should verify that the cart has all the items that were previously added', () => {
        ...
        cy.get(cartPage.getProductCardData(3)[1]).should("contain", `$${product1.price * product1.quantity}.00`) // check product price
        ...
        cy.get(cartPage.getProductCardData(2)[1]).should("contain", `$${product4.price * product4.quantity}.00`) // check product price
        ...
        cy.get(cartPage.getProductCardData(1)[1]).should("contain", `$${product6.price * product6.quantity}.00`) // check product price
     ...
     * Test Scenario: Check cart functionality
     */
    it.only('should verify that the user cannot modify the price or subtotal of the product', () => {
        // add multiple products to cart
        homePage.addToCart(1, product1.quantity)
        cy.wait(500)
        cartPage.closeCart()
        ...
        // assert that changes cannot be made
        cy.get(cartPage.cartTotal).should("")
